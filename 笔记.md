...
每个人线程中会维护一个ThreadLocalMap，key为ThreadLocal，value为ThreadLocal设置的值。
...
private void set(ThreadLocal<?> key, Object value) {
	...
	*/
    for (Entry e = tab[i];
		...
		//遇到key=null的过期节点，执行replaceStaleEntry并返回
        if (k == null) {
            replaceStaleEntry(key, value, i);
            return;
        }
    }
	//没有遇到相同hash冲突且不存在相同key，在空位置插入数据
...
private static int nextHashCode() {
    //HASH_INCREMENT`0x61c88647` 为斐波那契数，使用该数值作为 hash 增量可以使 hash 分布更加均匀
...
staleSlot为hash位置，从staleSlot向前探测过期节点，向后探测key是否已经存在，每次清理过期节点时会将节点重新hash并放置到位置或最近位置。
...
private void replaceStaleEntry(ThreadLocal<?> key, Object value,
    ...
    *   }
    */
    //从当前位置向前遍历,遍历到头后从尾继续向前遍历，直到遇到null节点停止
...
private int expungeStaleEntry(int staleSlot) {
    ...
    for (i = nextIndex(staleSlot, len);
         ...
         i = nextIndex(i, len)) {
        ThreadLocal<?> k = e.get();
        //key为null将Entry设置为null
        if (k == null) {
            e.value = null;
            tab[i] = null;
            size--;
        } else {
            //存在节点，将节点重新hash，放入位置或后面第一个null处
...
final int fullyRelease(Node node) {
    ...
    try {
        ...
        //当前节点一定获取到锁 
        为Sync Head，释放当前节点的锁，从Sync queue中移除
...
final boolean transferAfterCancelledWait(Node node) {
     ...
     * spin.
     */
    //已经解锁，但节点还没添加到Sync队列中，将线程让出等待添加成功
    while (!isOnSyncQueue(node))
        Thread.yield();
    return false;
}
...
将AQS中的state同时维护两个状态，高16位维护共享锁数量，低16位维护独占锁可重入数。

允许多把读锁同时读，写锁同时仅允许一把，读锁写锁互斥，需要等待另一种锁完全释放才能获取锁。
允许锁降级，不允许锁升级，在写锁中可以获得读锁。
...
Sync中重写的tryAcquireShared。

为什么引入firstReader以及firstReaderHoldCount

https://juejin.cn/post/6951566514017271821
...
protected final int tryAcquireShared(int unused) {
    ...
    //获取当前锁状态
    int c = getState();
    //当前独占锁状态不为0，并且获取独占锁的不是当前线程，返回失败
    ...
    //获取共享锁数量
    int r = sharedCount(c);
    //检查获取锁是否需要阻塞，检查读锁最大限制，更新高16位锁+1
    if (!readerShouldBlock() &&
        r < MAX_COUNT &&
        compareAndSetState(c, c + SHARED_UNIT)) {
        //第一个获取读锁的线程，不使用threadlocal维护重入数
        if (r == 0) {
            //初始化，将当前线程加入firstReader
            firstReader = current;
            //重置可重入计数
        ...
        //维护firstReader重入数
            ...
            //第二个及以后线程，使用ThreadLocal计数
            HoldCounter rh = cachedHoldCounter;
            //缓存的重入计数器与当前线程Id不同，缓存失效
...
并非完全的非公平实现，完全的非公平实现可以直接返回false
当前实现有一定概率防止读锁直接加入锁，写锁一直在队列中等待获取不到锁的情况
...
final int fullTryAcquireShared(Thread current) {
    ...
    for (;;) {
        ...
        //判读需要阻塞获取锁
			...
			//当前线程不是firstReader，可重入数在ThreadLocal中维护
            if (firstReader == current) {
                //此处不做处理是因为在firstReader在初始化时会重置firstReaderHoldCount
                //也可能是为了降低ThreadLocal对内存的消耗，将不涉及重入的线程先释放掉
            } else {
                if (rh == null) {
...
https://www.cnblogs.com/bmilk/p/13037655.html
...
```java
//获取读锁后对后续共享节点进行传播，将其唤醒并获取读锁
private void setHeadAndPropagate(Node node, int propagate) {
    Node h = head; // Record old head for check below
    //将当前节点设置为头节点
    setHead(node);
    //TODO
    //propagate > 0 表示还需释放多少资源
    //h == null || h.waitStatus < 0 || (h = head) == null 防止空指针，实际不会出现
    //h.waitStatus < 0 h状态为SINGAL PROPAGATE才会继续传播
...
private void doReleaseShared() {
    ...
    for (;;) {
        ...
        if (h != null && h != tail) {
            ...
            //第二次循环将节点设置为PROPAGATE
            else if (ws == 0 &&
                     !compareAndSetWaitStatus(h, 0, Node.PROPAGATE))
                continue;                // loop on failed CAS
        }
        //头节点没变化退出
        //head -> write1 -> read1 -> read2 -> write2
        //以上情况再释放read1的情况时会通过setHeadAndPropagate唤醒read2
        //read2会再次通过setHeadAndPropagate将read2设置为Head
        //unparkSuccessor会额外唤醒write2，但write2不会竞争到锁
...
扩容Semaphore许可可以直接调用release，Semaphore并没有验证许可数量
    ...
    try {
        ...
        for (;;) {
            try {
				...
				//中断时
                //当前代没有失效，也没有被损坏，当前代失效
                if (g == generation && ! g.broken) {
                    breakBarrier();
                    throw ie;
                } else {
                    //说明已经进入到下一代，当前线程不属于当前代，仅做中断处理
			...
			//当有任意一个线程中断，都会调用breakBarrier通知其他线程，使当前代失效，其他线程都要抛出异常
            if (g.broken)
                throw new BrokenBarrierException();
			//已经换代，属于正常返回
            if (g != generation)
...
**目前开发过程中测试存在的问题:**
 ...
 开发一个新功能后,通过本地前后端打开网页,在网页上操作一遍新功能,没有问题就提交代码.

 代码部署在218等服务器上,再操作一遍这次部署与自己相关的改动功能. 没有问题就让测试人员测.

 上面的过程中存在的主要问题是:

  自己在网页上操作时,通常是测正常情况,不会覆盖所有的边界情况/异常情况等.

  没有留下测试相关的文件,无法重复测试. 出现bug后修复后,又在网页上操作一遍.

  

在开发阶段引入单元测试的作用:

  粒度小,编写边界/异常情况的用例 比在网页上操作容易

  可重复执行, 可以放心重构. 也可以及时发现别人代码改动产生的影响

  

### 测试使用的组件

1. junit5 (包名是org.junit.jupiter ,不要使用junit4 或者 testng)
...
### 测试范围
 ...
 针对对一个public方法的测试,主要关注这个方法的代码逻辑

 针对一个局部功能的测试,主要关注这个小功能的返回结果



### 测试代码运行

 项目内的测试代码被分成两部分,一部分是普通测试,一部分是集成测试.
...
3. 设计用例,比如正常,异常,错误,边界,分支覆盖等用例数据
...
​       一个@Test方法对应一条测试用例,比如设计了正常的/错误的 两个用例,那应该在 两个@Test方法中

 

-   测试方法命名

​    一般正常的测试用例使用test{MethodName}命名, 其他的用例一般在名字里体现 test{MethodName}IfXXNotExist, test{MethodName}MaxValue



-  如何检测是否通过

​    使用assertTrue,assertFalse,assertEquals等方法. 



-  哪些方法需要添加测试

​    public方法, get set controller 这种方法可以不需要



-   需要读取文件

​       文件放在resources目录下,不要使用自己本地的绝对路径 PermissonVerifyTest



-  待测方法中调用了其他方法

​    一般其他方法如果调用了api,从数据库查数据这种依赖外部组件的, 可以 mock 这些方法的返回值.

​    如果调用的其他方法方法没有依赖外部组件,可以选择不mock



-  对一个方法应该测什么东西

​    测输入条件下方法的返回值是否符合预期,其实这种有点像黑盒测试,主要assert判断方法返回的值
...
​     需要确认是新代码的逻辑有问题, 还是测试用例需要更新



-  开发了一个新功能,在界面上操作了一遍,没有问题.这时候还需要写测试代码吗?

​     目前这种界面上人工操作的测试没有留下任何可重复的测试信息, 所以需要对这个新功能中比较核心的一些方法写测试代码,以便以后可以重复执行.
...
​      测试人员主要是更大型的模块间集成测试,整体的大功能测试. 与开发关注的测试范围不一样
