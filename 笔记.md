...
```
​	ApplicationClassLoader和ExtensionClassLoader都是在Launcher中初始化的。
...
URLClassLoader可以加载任意路径下的类，URLClassLoader继承于ClassLoader，是对于ClassLoader的扩展，ClassLoader只可以加载ClassPath下的类，URLClassLoader可以通过传入的URL地址进行加载，URLClassLoader中没有对loadClass()进行实现。ExtensionClassLoader，ApplicationClassLoader都继承与

自定义类加载器需要继承URLClassLoader，
